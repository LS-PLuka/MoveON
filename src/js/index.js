...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function criarPostElement(post) {
        ...
        <div class="comments-list">
          ${post.comentarios?.length
            ? post.comentarios.map(c => `
              <div class="comment">
                <span class="comment-user">${c.usuario?.nome || 'Anônimo'}:</span>
    ...
    commentForm.addEventListener('submit', async (e) => {
      ...
      const usuarioId = localStorage.getItem('usuario_id');
      const nomeUsuario = localStorage.getItem('usuario_nome') || 'Você';
      ...
      try {
        ...
        const respostaAtualizada = await fetch(`http://localhost:3000/api/postagens/${postId}/comentarios`);
        const comentarios = await respostaAtualizada.json();
